...
export function DashboardStats() {
  ...
  const stats = [
    {
      title: "Actieve Schepen",
      value: Object.values(shipDatabase).filter((ship) => ship.status === "Operationeel").length.toString(),
      description: "2 in aanbouw (Liberte, Fraternite)",
      icon: Ship,
      color: "text-blue-600",
      bgColor: "bg-blue-100",
    },
    {
      title: "Totaal Bemanning",
      value: total.toString(),
      description: "Internationale bemanning",
      icon: Users,
  ...
  const statLinks = [
    "/schepen", // Actieve Schepen
    "/bemanning", // Totaal Bemanning
    "/ziekte", // Ziekmeldingen
  ]
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      ...
      <Link href="/bemanning/aflossers" className="bg-green-50 border border-green-200 rounded-lg p-4 text-center hover:bg-green-100 transition cursor-pointer">
        <div className="text-2xl font-bold text-green-800">{aflossers}</div>
      ...
      <Link href="/bemanning/studenten" className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-center hover:bg-purple-100 transition cursor-pointer">
        <div className="text-2xl font-bold text-purple-800">{studenten}</div>
      ...
      <Link href="/bemanning/nog-in-te-delen" className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center hover:bg-gray-100 transition cursor-pointer">
        <div className="text-2xl font-bold text-gray-800">{nogInTeDelen}</div>
