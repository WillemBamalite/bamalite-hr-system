...
export default function NogInTeDelenPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        console.log("🔍 Out-of-service data:", outOfServiceData ? JSON.parse(outOfServiceData) : []);
      } catch (error) {
  ...
  const crew = Object.values(allCrewData || {}).filter((c: any) => {
    // Debug logging
    if (c && c.firstName === "test" && c.lastName === "1") {
      console.log("🔍 Debug test-1:", {
        id: c.id,
        status: c.status,
        shipId: c.shipId,
        isAflosser: c.isAflosser,
        position: c.position,
        function: c.function
      });
    }
  ...
  const operationalShips = Object.values(shipDatabase).filter((ship: any) => ship.status === "Operationeel");
  ...
  const assignToShip = () => {
    ...
    try {
      ...
      const updatedMember = {
        ...
        assignmentHistory: [
          ...(fullProfile.assignmentHistory || []),
          {
            ...
            shipId: selectedShip,
            action: "toegewezen",
            note: `Toegewezen aan ${(shipDatabase as any)[selectedShip]?.name || "Onbekend schip"} met status: ${initialStatus}`,
          }
        ]
      };
      ...
      const shipName = (shipDatabase as any)[selectedShip]?.name || "Onbekend schip";
