...
export default function SimpleAflossersPage() {
  ...
  function handleCreateAssignment() {
    ...
    // Toon melding
    const shipName = shipDatabase[newAssignment.shipId as keyof typeof shipDatabase]?.name || "Onbekend schip";
                  ...
                  {getAflosserHistory(selectedAflosser?.id).map((assignment) => (
                    <Card key={assignment.id}>
                      <CardContent className="pt-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="font-medium">{shipDatabase[assignment.shipId as keyof typeof shipDatabase]?.name}</div>
