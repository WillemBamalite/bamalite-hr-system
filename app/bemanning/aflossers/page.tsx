...
export default function ReizenAflossersPage() {
  ...
  const getCurrentAbsence = (aflosser: any) => {
    if (!aflosser.notes || aflosser.notes.length === 0) return null
  ...
  const handleAssignAflosser = async () => {
    ...
    try {
      // Get trip and aflosser info
      const trip = trips.find((t: any) => t.id === assignAflosserDialog)
      const selectedAflosser = crew.find((c: any) => c.id === selectedAflosserId)
      
      if (!trip || !selectedAflosser) {
        alert("Fout: reis of aflosser niet gevonden")
        return
      }
      ...
      // Check if aflosser has absences
      if (selectedAflosser.notes && selectedAflosser.notes.length > 0) {
        const absenceNotes = selectedAflosser.notes.filter((note: any) => {
          const noteText = typeof note === 'string' ? note : note?.content || note?.text || ''
          return noteText.includes('AFWEZIGHEID:')
        })
          ...
          try {
            ...
            if (jsonMatch) {
              ...
              if (Array.isArray(absences) && absences.length > 0) {
                ...
                // Calculate 2 weeks after trip start
                const twoWeeksAfterTripStart = new Date(tripStartDate)
                twoWeeksAfterTripStart.setDate(twoWeeksAfterTripStart.getDate() + 14)
                
                // Find conflicting absence
                const conflictingAbsence = absences.find((absence: any) => {
                  ...
                  absenceToDate.setHours(0, 0, 0, 0)
                  
                  // Check if currently absent
                  const isCurrentlyAbsent = absenceFromDate <= today && absenceToDate >= today
                  
                  // Check if absent within 2 weeks after trip start
                  const isAbsentWithin2Weeks = absenceFromDate <= twoWeeksAfterTripStart && absenceToDate >= tripStartDate
                  
                  return isCurrentlyAbsent || isAbsentWithin2Weeks
                })
                
                if (conflictingAbsence) {
                  const absenceFromDate = new Date(conflictingAbsence.fromDate)
                  absenceFromDate.setHours(0, 0, 0, 0)
                  const absenceToDate = new Date(conflictingAbsence.toDate)
                  absenceToDate.setHours(0, 0, 0, 0)
                  
                  const isCurrentlyAbsent = absenceFromDate <= today && absenceToDate >= today
                  
                  warningMessage = `⚠️ Waarschuwing: ${selectedAflosser.first_name} ${selectedAflosser.last_name} is ${isCurrentlyAbsent ? 'momenteel' : 'binnen 2 weken na de startdatum'} afwezig van ${format(absenceFromDate, 'dd-MM-yyyy')} tot ${format(absenceToDate, 'dd-MM-yyyy')}${conflictingAbsence.reason ? ` (${conflictingAbsence.reason})` : ''}.`
                }
              }
            }
          } catch (e) {
      ...
      // Show warning after successful assignment if there is one
      if (warningMessage) {
        alert(warningMessage)
      }
      // Aflosser assigned successfully
    } catch (error) {
  ...
  const handleEditTrip = async () => {
    ...
    try {
      // Get trip info
      const trip = trips.find((t: any) => t.id === editTripDialog)
      if (!trip) {
        alert("Fout: reis niet gevonden")
        return
      }

      // Check for conflicts if assigning a new aflosser
      let warningMessage = ""
      if (editTripData.aflosser_id && editTripData.aflosser_id !== "none") {
        const selectedAflosser = crew.find((c: any) => c.id === editTripData.aflosser_id)
      ...
      // Show warning after successful update if there is one
      if (warningMessage) {
        alert(warningMessage)
      } else {
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                            ...
                            {getCurrentAbsence(aflosser) && (() => {
                              const absence = getCurrentAbsence(aflosser)
                              return (
                                <div className="bg-orange-50 border border-orange-200 p-3 rounded-lg">
                                  <div className="flex items-start space-x-2">
                                    <UserX className="w-4 h-4 text-orange-600 mt-0.5" />
                                    <div className="flex-1">
                                      <p className="text-sm font-medium text-orange-800 mb-1">Momenteel afwezig</p>
