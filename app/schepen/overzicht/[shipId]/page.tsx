...
export default function ShipParticularsPage() {
  ...
  useEffect(() => {
    ...
    const syncCertificateState = async () => {
      ...
      if (cloudShipState) {
        ...
        await saveCloudShipState(ship.name, localStoredArray, localDocuments, localRemoved)
        ...
        if (refreshedCloudState) {
          ...
          const removedSet = new Set(localRemoved)
          const filteredDefaults = defaults.filter(
            (item) => !removedSet.has(normalizeCertificateName(item.naam))
          )
          ...
          setCertificatesDirty(hasUnsavedLocalChanges)
        }
      } else {
        const defaults = getShipCertificateDefaultsForClient(ship.name)
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !ship?.name) return
    if (certificatesDirty || certificatesSaving || !certificatesSyncReadyRef.current) return
    let cancelled = false
