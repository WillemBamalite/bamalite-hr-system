...
export default function ZiektePage() {
  ...
  const noCertificateOrExpired = sickLeaveRecords.filter((record: any) => {
    // Status "wacht-op-briefje" gaat naar eigen kolom
  ...
  const waitingForCertificateRecords = sickLeaveRecords.filter((record: any) => 
    record.status === "wacht-op-briefje"
  )
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-2">
      ...
      <div className="hidden md:block space-y-8">
        ...
        {/* Sectie 2: Wacht op ziektebriefje */}
        <div className="space-y-4">
          <div className="pb-4 border-b">
            <h2 className="text-lg font-semibold text-orange-800">Wacht op ziektebriefje</h2>
            <p className="text-sm text-gray-500 mt-1">{waitingForCertificateRecords.length} {waitingForCertificateRecords.length === 1 ? 'record' : 'records'}</p>
          </div>
          {waitingForCertificateRecords.length > 0 ? (
            <div className="grid grid-cols-3 gap-6">
              {waitingForCertificateRecords.map((record: any) => renderRecordCard(record))}
            </div>
          ) : (
            <div className="text-center py-8 text-gray-400 text-sm">
